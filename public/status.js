...
function renderTradingControl() {
    ...
    if (isReadyToGoLive) {
        ...
        if (note) note.textContent = '检测到您处于真实视图，点击将自动切为实盘模式并开始交易。';
    } else if (isReadyToGoPaper) {
        btn.textContent = '切换并启动 [模拟]';
        if (note) note.textContent = '检测到您处于模拟视图，点击将自动切回模拟模式并开启。';
    } else {
        const modeLabel = isRealView ? '[实盘]' : '[模拟]';
        if (dashboardState.tradingEnabled) {
            btn.textContent = `${modeLabel} 运行中`;
            if (note) note.textContent = '当前机器人正在自动运行；关闭后不再新开仓，现有持仓继续按规则处理。';
        } else {
            btn.textContent = `启动 ${modeLabel} 交易`;
            if (note) note.textContent = `当前 ${modeLabel} 交易已关闭；点击按钮即可恢复自动化运行。`;
        }
    }
...
async function fetchBotStatus() {
    try {
        ...
        // renderAiThoughts/updateDecision 被移除
    ...
    list.innerHTML = entries.slice(0, 15).map((entry, idx) => {
        ...
        if (isLatest) {
            return `<div class="ai-history-card is-latest">
                ...
                <div class="ai-history-columns" style="display: none;"></div> <!-- 详细模式下移除冗余栏目 -->
            </div>`;
        }
...
function renderCapitalPanel(data) {
    ...
    if (winRateEl) {
        winRateEl.className = `perf-val mono ${winRate >= 0.5 ? 'c-green' : winRate > 0 ? 'c-amber' : ''}`;
    }
...
// 覆写原版 renderTradingControl 为高区分度智能模式
function renderTradingControl() {
    const btn = document.getElementById('trade-toggle-btn');
    const note = document.getElementById('trade-toggle-note');
    if (!btn) return;
...
async function toggleTrading() {
    ...
    try {
        ...
        if (isReadyToGoLive || isReadyToGoPaper) {
            ...
            console.log(`🔄 正在请求切换至 ${targetMode} 模式...`);
